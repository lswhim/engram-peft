...
OUT=$ROOT/outputs/qwen3_scratch_1p7b_pt_1b/runs
...
for mode in base arithmetic semantic_flatten semantic_keyed; do
  ...
  if [[ -s "$mode_out/metrics.json" ]]; then
    echo "[qwen3 scratch queue] already complete: $mode" | tee -a "$OUT/queue.log"
    continue
  fi
  mkdir -p "$mode_out"
  extra_args=()
  ...
  echo "[qwen3 scratch queue] starting mode=$mode" | tee -a "$OUT/queue.log" "$log"
  CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 "$TORCHRUN" \
    --standalone --nproc_per_node=8 \
    examples/run_qwen3_scratch_1p7b_pt_1b.py \
    --mode "$mode" \
    --data-dir "$DATA" \
    --tokenizer "$MODEL" \
    --output-dir "$mode_out" \
    ...
    "${extra_args[@]}" \
    >> "$log" 2>&1
  echo "[qwen3 scratch queue] finished mode=$mode" | tee -a "$OUT/queue.log"
done

echo "QWEN3_SCRATCH_1P7B_PT_1B_QUEUE_DONE" | tee -a "$OUT/queue.log"
