...
SOURCE=rq_tables/pararel5k_qwen3emb8b_M8K16

cd "$ROOT"
export CUDA_VISIBLE_DEVICES="$GPU_ID"
export HF_HUB_OFFLINE=1
export PYTHONPATH="$ROOT/src:$ROOT"
mkdir -p run_logs outputs/semantic_memory/pararel_embedder_sweep
...
if [[ "$MODE" == semantic ]]; then
  "$PY" -u scripts/build_rq_table.py \
    ...
    --num_docs 5000 --base_tokenizer "$BASE" --embedder "$EMBEDDER" \
    --num_levels 8 --codebook_size 16 --projection_dim 0 \
    --max_ngrams_per_size 300000 --min_count 1 --output_dir "$SOURCE"
  TABLE="$SOURCE"
  RUN=pararel5k_qwen3emb8b_k16_semantic_seed42
  RESULT=outputs/semantic_memory/pararel_embedder_sweep/qwen3emb8b_semantic_k16_seed42.json
elif [[ "$MODE" == shuffled ]]; then
  while [[ ! -f "$SOURCE/meta.json" ]]; do sleep 15; done
  TABLE=rq_tables/pararel5k_qwen3emb8b_M8K16_runtime_shuffled_seed42
  if [[ ! -f "$TABLE/meta.json" ]]; then
    "$PY" scripts/make_runtime_shuffled_rq.py --source "$SOURCE" --output "$TABLE" --seed 42
  fi
  RUN=pararel5k_qwen3emb8b_k16_shuffled_seed42
  RESULT=outputs/semantic_memory/pararel_embedder_sweep/qwen3emb8b_shuffled_k16_seed42.json
else
