...
SOURCE=rq_tables/pararel5k_qwen3emb4b_M8K16
...
if [[ "$MODE" == semantic ]]; then
  ...
  TABLE="$SOURCE"
  RUN=pararel5k_qwen3emb4b_k16_semantic_seed42
  RESULT=outputs/semantic_memory/pararel_embedder_sweep/qwen3emb4b_semantic_k16_seed42.json
elif [[ "$MODE" == shuffled ]]; then
  while [[ ! -f "$SOURCE/meta.json" ]]; do sleep 15; done
  TABLE=rq_tables/pararel5k_qwen3emb4b_M8K16_runtime_shuffled_seed42
  if [[ ! -f "$TABLE/meta.json" ]]; then
    "$PY" scripts/make_runtime_shuffled_rq.py --source "$SOURCE" --output "$TABLE" --seed 42
  fi
  RUN=pararel5k_qwen3emb4b_k16_shuffled_seed42
  RESULT=outputs/semantic_memory/pararel_embedder_sweep/qwen3emb4b_shuffled_k16_seed42.json
else
