...
Strict load-matched flatten 三 seed 已完成。Semantic-flat − load-matched-flat 的 Efficacy/Generalization 在
1K、5K、10K、50K 均显著为正：`+0.838/+1.340`、`+0.968/+0.787`、`+1.214/+1.227`、
`+1.014/+1.505` pp。50K hierarchical paired case-bootstrap CI 为 Efficacy `[+0.575,+1.458]`、
Generalization `[+1.110,+1.904]`；Locality `+0.409` pp，CI `[−0.007,+0.813]`；Multi-hop `+0.069` pp，
CI `[−0.731,+0.770]`。该控制在精确相同 train-access frequency 的 rows 内置换完整 code vectors，保留 joint
multiset、逐级 histogram 与 access-weighted bucket loads；因此 Efficacy/Generalization 的稳定差异是当前最强的
semantic ownership 因果证据，而 locality/multi-hop 暂不支持显著主张。
