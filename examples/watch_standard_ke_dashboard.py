...
def aggregate_rows(root: Path) -> str:
    ...
    for dataset in ('counterfact','zsre'):
        for method in ('arithmetic','lora','rq'):
            ...
            row_class=f'method-{method}' + (' dataset-start' if dataset == 'zsre' and method == 'arithmetic' else '')
            rows.append(f'<tr class="{row_class}"><th>{"CounterFact" if dataset=="counterfact" else "ZsRE"}</th><td>{METHOD_LABELS[method]}</td><td>{len(runs)}/3</td>'+''.join(f'<td>{v}</td>' for v in values)+'</tr>')
    return ''.join(rows)
