...
def conclusions(root: Path) -> str:
    ...
    for ds,label in [('counterfact','CounterFact'),('zsre','ZsRE')]:
        a=results.get(f'{ds}_arithmetic_seed42'); l=results.get(f'{ds}_lora_seed42')
        if a and l:
            am,lm=a['metrics'],l['metrics']
            items.append(f'<li><b>{label}：</b>LoRA 的 efficacy/paraphrase 为 {100*lm["efficacy"]:.2f}/{100*lm["paraphrase"]:.2f}，Arithmetic-fixed 为 {100*am["efficacy"]:.2f}/{100*am["paraphrase"]:.2f}；specificity 分别为 {100*lm["specificity"]:.2f}/{100*am["specificity"]:.2f}。这说明当前存在写入强度与局部保持的 trade-off。</li>')
