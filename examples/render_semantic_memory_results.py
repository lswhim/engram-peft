...
def main() -> None:
    ...
    control_specs=(
        ("Semantic-RQ",args.root/"pararel_k_sweep"/"k16_seed42.json"),
        ("RQ-Shuffled",args.root/"pararel_k16_controls"/"shuffled_seed42.json"),
        ("Arithmetic-fixed",args.root/"pararel_k16_controls"/"arithmetic_fixed_seed42.json"),
    )
    control_rows="".join(
        f"<tr><th>{label}</th><td>16</td><td>8</td><td>{100*load(path)['metrics']['role/should_propagate']['mean']:.2f}</td></tr>"
        if path.is_file() else f"<tr><th>{label}</th><td>16</td><td>8</td><td>运行中</td></tr>"
        for label,path in control_specs
    )
    embedder_specs=(
        ("Qwen3-Embedding-0.6B","Semantic-RQ",args.root/"pararel_k_sweep"/"k16_seed42.json"),
    ...
    embedder_rows="".join(
        ...
        if path.is_file() else f"<tr><th>{embedder}</th><td>{method}</td><td>16</td><td>运行中</td></tr>"
        for embedder,method,path in embedder_specs
    )
    corpus_specs=(
        ("ParaRel canonical 5K","Semantic-RQ",args.root/"pararel_k_sweep"/"k16_seed42.json"),
        ("ParaRel canonical 5K","RQ-Shuffled",args.root/"pararel_k16_controls"/"shuffled_seed42.json"),
        ("FineWeb-Edu 12K docs / 300K n-grams","Semantic-RQ",args.root/"pararel_fineweb_k16"/"semantic_seed42.json"),
        ("FineWeb-Edu 12K docs / 300K n-grams","RQ-Shuffled",args.root/"pararel_fineweb_k16"/"shuffled_seed42.json"),
    )
    corpus_rows="".join(
        f"<tr><th>{corpus}</th><td>{method}</td><td>0.6B</td><td>16</td><td>{100*load(path)['metrics']['role/should_propagate']['mean']:.2f}</td></tr>"
        if path.is_file() else f"<tr><th>{corpus}</th><td>{method}</td><td>0.6B</td><td>16</td><td>运行中</td></tr>"
        for corpus,method,path in corpus_specs
    )
    k_section=f"""<section><h2>ParaRel in-domain RQ · Codebook-size sweep</h2><p>固定 M=8、5K canonical writes、seed 42；每种 K 在独立 GPU 上完成建表、782-step 训练和 16,024-query unseen-template 评测。地址行数为 2/3-gram 两组 memory heads 每层的合计，不表示组合空间。</p><table><thead><tr><th>K / level</th><th>RQ levels</th><th>Rows / level</th><th>Accuracy (%)</th><th>Δ FineWeb-RQ</th><th>Δ Arithmetic</th></tr></thead><tbody>{k_rows}</tbody></table><h3>K=16 因果对照</h3><p>三种方法严格匹配为每个 n-gram 8 heads × 每 head 16 rows。Shuffled 保留 RQ code 分布但破坏语义排列；Arithmetic-fixed 使用独立随机算术地址。</p><table><thead><tr><th>Method</th><th>Rows / head</th><th>Heads / n-gram</th><th>Accuracy (%)</th></tr></thead><tbody>{control_rows}</tbody></table><h3>Embedding model sweep · K=16</h3><table><thead><tr><th>Address encoder</th><th>Address geometry</th><th>K</th><th>Accuracy (%)</th></tr></thead><tbody>{embedder_rows}</tbody></table><h3>Address-corpus sweep · Qwen3-Embedding-0.6B / K=16</h3><table><thead><tr><th>RQ training corpus</th><th>Address geometry</th><th>Encoder</th><th>K</th><th>Accuracy (%)</th></tr></thead><tbody>{corpus_rows}</tbody></table></section>"""
