...
<section class="verdicts"><div class="verdict primary"><b>旧实现审计</b><strong>100K+Arithmetic fallback，不判定完整方法</strong><p class="muted">下列 Gate A / B 数值来自旧的有限预计算词典；表外 n-gram 被错误回退到 Arithmetic。结果保留作受限实现消融，不再用于否定 all-n-gram Semantic-RQ。</p></div><div class="verdict"><b>正确实现 · 修复中</b><strong>首次 miss 语义编码并持久缓存</strong><p class="muted">Frozen Qwen3-Embedding → frozen RQ → 当前首次访问立即使用；热缓存纯查表，禁止 Arithmetic fallback。</p></div><div class="verdict"><b>旧 Gate A · 边界证据</b><strong>{html.escape(public_verdict)}</strong><p class="muted">{html.escape(public_detail)}</p></div><div class="verdict"><b>旧 Gate B · 边界证据</b><strong>{html.escape(gate_b_verdict)}</strong><p class="muted">{html.escape(gate_b_detail)}</p></div></section>
