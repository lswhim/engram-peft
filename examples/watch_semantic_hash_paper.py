...
def render(snapshot: dict[str, Any]) -> str:
    ...
    if arithmetic42_wiki is not None and arithmetic42_lambada_acc is not None:
...
<section class="verdicts"><div class="verdict primary"><b>当前总判断</b><strong>{html.escape(public_verdict)}</strong><p class="muted">{html.escape(public_detail)}</p></div><div class="verdict"><b>XNLI 附录诊断</b><strong>基本持平</strong><p class="muted">{html.escape(xnli_summary)}</p></div><div class="verdict"><b>地址几何</b><strong>结构信号明确</strong><p class="muted">2g ρ=.738 vs .006；3g ρ=.699 vs −.011。不能替代 benchmark。</p></div><div class="verdict"><b>下一决策点</b><strong>Semantic / Shuffled 对照</strong><p class="muted">判断退化来自 memory 本身，还是 Arithmetic 寻址。</p></div></section>
<h2>阶段 1 到底怎么训练？</h2><p class="muted"><b>准确定位：memory-only continued pretraining（CPT）。</b> 新建的地址只告诉 n-gram 去哪个 slot，但 slot 里的 value 起初是随机的；所以冻结 Qwen，用 causal LM 目标把 memory value 学出来。这是训练新增 memory / adapter 模块的标准做法，但当前 1 亿 token slots 的规模不是完整的大模型 CPT，也不能单独证明方法有效。</p><div class="flow"><article><b>1 · 训练文本</b><p>FineWeb-Edu：48,832 条序列，每条最多 256 tokens。</p></article><article><b>2 · 冻结底座</b><p>Qwen3-1.7B 全部冻结，只更新约 27M Engram 参数。</p></article><article><b>3 · 查 memory</b><p>当前位置的 2-gram / 3-gram 经各自地址函数找到 memory slot。</p></article><article><b>4 · 注入并预测</b><p>读取 memory 向量注入 Qwen，用标准 causal LM loss 预测下一 token。</p></article><article><b>5 · 跑满并保存</b><p>batch 4、累积 8、12,208 steps、8 epochs；保存完整 checkpoint。</p></article></div><div class="method-grid"><article><h3>Arithmetic-fixed</h3><p>token ID → 固定算术 hash → slot。语义相近短语不会刻意靠近，是原始随机离散寻址基线。</p></article><article><h3>Semantic-RQ（我们的方法）</h3><p>离线 n-gram embedding → residual quantization code → slot。地址表和 codebook 固定，训练期只学习 memory value。</p></article><article><h3>RQ-Shuffled（关键因果对照）</h3><p>保留 Semantic-RQ 的容量与访问频率，再打乱 n-gram↔地址关系，用来单独消除语义几何。</p></article></div><p class="muted"><b>为什么是 9 个？</b> 三种寻址 × seeds 42/43/44。seed 改变 memory 初始化和数据顺序；最终报告均值和方差。<b>阶段 1 只负责让随机 memory 学起来并控制训练预算；能否迁移到 WikiText、C4、LAMBADA 与 PAWS，才是论文结论。</b></p>
<h2>现在每个实验在做什么</h2><div class="experiments"><article class="experiment"><h3>① 公平 checkpoint 训练 <span class="pill run">{completed_checkpoint_count}/9 已完成</span></h3><dl><dt>目的</dt><dd>获得三种地址方法可直接比较的完整模型。</dd><dt>协议</dt><dd>冻结 Qwen3-1.7B，只训练 Engram；FineWeb-Edu；12,208 steps；3 seeds。</dd><dt>对照</dt><dd>Arithmetic-fixed、RQ-Shuffled、Semantic-RQ；参数与表容量严格一致。</dd><dt>成功标准</dt><dd>这里只产出 checkpoint，不用不同 step 的中间 loss 下结论。</dd></dl></article><article class="experiment"><h3>② Base 公开 LM 基线 {base_status_badge}</h3><dl><dt>目的</dt><dd>测没有 Engram 时 Qwen3-1.7B 的公开 benchmark 基准线。</dd><dt>结果/状态</dt><dd>{base_status_text}</dd><dt>协议</dt><dd>WikiText-103、C4 validation 与 LAMBADA 均使用公开标准任务。</dd><dt>对照</dt><dd>后续所有 Arithmetic、Shuffled、Semantic checkpoint 与同一 Base 比较。</dd></dl></article><article class="experiment"><h3>③ Gate A：标准语言建模 <span class="pill wait">{completed_checkpoint_count}/9 checkpoint 可评测</span></h3><dl><dt>目的</dt><dd>直接判断 Semantic Hash 是否带来公开 LM 收益。</dd><dt>当前状态</dt><dd>完整 checkpoint 已排队；lm-eval 的自动 batch 探测接近占满 40GB，需要等一张独占评测卡，不能与仍在训练的模型强行共卡。</dd><dt>任务</dt><dd>WikiText-103、C4 validation、LAMBADA、FineWeb held-out。</dd><dt>成功标准</dt><dd>Semantic-RQ 同时优于 Arithmetic 和 Shuffled，至少两个 benchmark 方向一致。</dd><dt>失败处理</dt><dd>若持平或退化，不用地址几何包装成方法提升。</dd></dl></article><article class="experiment"><h3>④ Gate B：QQP → PAWS <span class="pill wait">尚未启动</span></h3><dl><dt>目的</dt><dd>验证公开 paraphrase OOD 泛化，而不是自建“语义泛化”样本。</dd><dt>协议</dt><dd>QQP 训练；QQP in-domain 与 PAWS-QQP zero-update 测试。</dd><dt>对照</dt><dd>Base、Arithmetic、RQ-Shuffled、Semantic-RQ，各 3 seeds。</dd><dt>成功标准</dt><dd>PAWS 提升且 QQP 不退化；否则不能 claim 语义泛化。</dd></dl></article></div>
