...
<section class="verdicts"><div class="verdict primary"><b>旧实现审计</b><strong>Fallback + code 解码错误，RQ 结果全部作废</strong><p class="muted">旧 Semantic/RQ-Shuffled 同时存在表外 Arithmetic fallback 与 FAISS bitstring 端序解析错误；仅 Arithmetic 对照仍有效。</p></div><div class="verdict"><b>正确实现 · 正式重训</b><strong>All-n-gram Semantic-RQ · seeds 42/43/44 已启动</strong><p class="muted">首次 miss：Frozen Qwen3-Embedding → frozen M8/K256 RQ → 当前访问立即使用；节点本地持久 cache，禁止 Arithmetic fallback。</p></div><div class="verdict"><b>集成门槛</b><strong>三 seed 20-step 全部通过</strong><p class="muted">约 4.98–5.50 s/step，峰值 8.37 GB；冷/热 code bitwise 一致、范围 0–255。该结果仅证明实现可训练，不作效果结论。</p></div><div class="verdict"><b>下一次有效结论</b><strong>等待正确 checkpoint 的公开 benchmark</strong><p class="muted">WikiText、LAMBADA、C4 与 PAWS 将只接受 strict all-n-gram checkpoint；旧 RQ 数值不再比较。</p></div></section>
